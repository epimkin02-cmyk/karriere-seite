...
/**
 * 90% white below `lg`, the template's 80% from `lg` up.
 *
 * The 80% frosting was tuned to let the WebGL scene glow through the header on
 * a desktop hero. On a phone the header floats over ordinary body copy for the
 * whole page, and at 80% that copy ghosts straight through the panel — headings
 * scrolling under the wordmark stay legible enough to read, which makes the bar
 * look broken rather than translucent. The scene only sits behind the header on
 * wide screens anyway, so the stronger fill costs nothing where the effect was
 * actually designed to be seen.
 */
const GLASS_PANEL =
  ...
  <header className="pointer-events-none fixed inset-x-0 top-4 z-50 px-5 md:px-10">
    <div className="flex items-center justify-between">
      ...
      <div className="hidden lg:block">
        <HeaderCta cta={cta} hideWhenVisibleId="bewerben" />
      </div>
