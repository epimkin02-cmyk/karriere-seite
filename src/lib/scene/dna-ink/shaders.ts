...
const SNOISE = /* glsl */ `
  ...
  vec3 helixPoint(float rnd1, float rnd2, float rnd3, float rnd4, float twist, float radius, float height, float thick, float wave, float time) {
    ...
    // TWO STRANDS, NO RUNGS.
    //
    // The original sketch spent 15% of its points on base-pair rungs — the
    // horizontal ladder steps between the strands — and those rungs are the
    // entire reason the shape reads as DNA rather than as two intertwined
    // lines. On a tax firm's careers page a double helix is a borrowed motif
    // from the clinic this template was designed for; two strands winding
    // around each other are just two paths that stay together, which is the
    // thing the page is actually about.
    //
    // Deleting the branch rather than setting the threshold to 1.0 matters for
    // more than tidiness: rnd1 now only chooses which of the two strands a
    // point belongs to, so the distribution is even instead of an 85/15 split,
    // and the strands come out denser at the same helixCount.
    //
    // (No backticks in this comment on purpose — the whole shader lives inside
    // a JS template literal, so one would end the string mid-GLSL.)
    float strand = step(0.5, rnd1) * 3.14159265;
    float a = y * twist + strand;
    vec3 core = vec3(radius * cos(a), y, radius * sin(a));
