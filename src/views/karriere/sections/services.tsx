...
import type { SectionIntro, ServiceCard } from "@/data/mocks/karriere";
import {
  BODY_MOTION,
  EYEBROW_MOTION,
  HEADING_MOTION,
...
const CARD_VARIANT: Record<ServiceCard["variant"], string> = {
  ...
  // `bg-foreground` under the photo is not decoration — it is the failure mode.
  // The card's copy is white and its only ground is the image; if that image
  // 404s, is still decoding, or is blocked, white text lands on the white page
        ...
        >
          {card.image && (
            <>
              ...
              />
              {/* The photo cards carry white copy directly on the picture. A
                  photograph is not a contrast guarantee — a bright window or a
                  white shirt behind a line of body text drops it below AA and
                  the failure is invisible until it lands on a real photo. The
                  scrim is heaviest at the bottom, where the body copy sits, and
                  clears the top so the image still reads as an image. */}
              <span
                aria-hidden="true"
                className="absolute inset-0 bg-gradient-to-t from-black/75 via-black/45 to-black/15"
              />
            </>
          )}
          ...
          {/* The benefit itself. The template's cards were pure titles because
              a clinic's services explain themselves; "600 € Gesundheitsbudget"
              does not — the sentence under it is what makes it worth reading,
              so it is the card, not a caption. Static rather than revealed:
              five cards each animating their own copy while the rail scrubs
              sideways reads as noise. */}
          {card.body && (
            <p
              lang="de"
              className="relative z-10 mt-auto text-sm leading-body font-light hyphens-auto lg:text-base"
            >
              {card.body}
            </p>
          )}
