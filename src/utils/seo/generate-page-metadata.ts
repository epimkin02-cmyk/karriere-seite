...
export function generateMetadata({
  ...
  return {
    ...
    openGraph: {
      ...
      url,
      siteName,
      // Must match what `app/opengraph-image.tsx` actually renders.
    ...
    // No `icons` block: `app/icon.tsx` and `app/apple-icon.tsx` are file
    // conventions, so Next emits the <link rel="icon"> tags itself. Declaring
    // them here as well would duplicate the tags and point at the starter's
    // deleted placeholder PNGs.
    manifest: "/manifest.json",
    // Driven by the same flag as `robots.ts`, so the meta tag and robots.txt
    // can never contradict each other. `nocache` and the Google-specific
    // directives are what actually keep an already-crawled URL out of the
    // index — `noindex` alone does not remove a cached snippet.
